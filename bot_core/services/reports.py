...
def _looks_like_error_or_login_page(html: str) -> bool:
    ...
    strong_markers = (
        ...
        "access denied",
        "request blocked",
        # Carfax-specific error pages (seen in production as PDFs).
        "uh-oh",
        "window sticker",
...
def _extract_safe_viewer_url(json_payload: Any, html_candidate: Optional[str]) -> Optional[str]:
    ...
    def _is_safe_url(u: str) -> bool:
        ...
        # Reject known non-VHR flows that can trigger downloads / sticker errors.
        if "/phoenix/" in path or "sticker" in path or "window" in path:
            return False
        ...
        # Only accept URLs that clearly look like VHR/report pages.
                ...
                for upstream_attempt in (1, 2, 3):
                    ...
                    if requested_lang != "en" and isinstance(viewer_url, str) and viewer_url:
                        try:
                            ...
                            if fast_mode:
                                fetch_html_budget_ms = min(fetch_html_budget_ms, 6_000)
